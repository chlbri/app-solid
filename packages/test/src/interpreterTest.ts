...
import type { Interpreter } from '@bemedev/app-solid';
import { tuple } from './tuple';

type DefaultSelector = <Input, Output>(value: Input) => Output;

const defaultSelector: DefaultSelector = value => value as any;

type State_F<T> = <const U = T>(
  accessor?: (state: T) => U,
  equals?: false | ((prev: U, next: U) => boolean),
) => Accessor<U>;

type StateSignal<M extends AnyMachine, S extends Ru> = {
  context: M['context'];
  value: StateValue;
  tags?: SoA<string>;
  event: M['__events'];
